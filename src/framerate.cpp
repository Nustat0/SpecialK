...
SK_ImGui_LatentSyncConfig (void)
{
  if (config.render.framerate.present_interval == 0)
  {
    ...
    if (config.render.framerate.target_fps > 0.0f)
    {
      ...
      if (bAdvanced)
      {
        bool bResyncRateInSeconds           = ! std::signbit (
          config.render.framerate.latent_sync.scanline_resync
        );

        static float fLastResyncRateFrames  = !bResyncRateInSeconds
          ? config.render.framerate.latent_sync.scanline_resync
          : -750.0f;

        static float fLastResyncRateSeconds =  bResyncRateInSeconds
          ? config.render.framerate.latent_sync.scanline_resync
          : 30.0f;
        ...
        if (ImGui::Checkbox ("Resync Rate in seconds", &bResyncRateInSeconds))
        {
          config.render.framerate.latent_sync.scanline_resync =
            bResyncRateInSeconds
              ? fLastResyncRateSeconds
              : fLastResyncRateFrames;
        }

        float fResyncRate = fabs (
          config.render.framerate.latent_sync.scanline_resync
        );

        if ( ImGui::SliderFloat ( "Resync Rate",
                                    &fResyncRate,
                                      0.0f, 1000.0f, bResyncRateInSeconds
                                                      ? "%.0f Second(s)"
                                                      : "%.0f Frame(s)" ) )
        {
          fResyncRate = std::max (0.0f, fResyncRate);
          ...
          config.render.framerate.latent_sync.scanline_resync = fResyncRate;
        }
